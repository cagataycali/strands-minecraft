 ...
 *
 * Audio tokens in → audio tokens out, semantic VAD, barge-in, mid-speech tool
 * calls. It is deliberately NOT the push-to-talk rail (voice.ts) scaled up.
 * That path is listen → transcribe → think → say, four serial round trips with
 * a transcription bottleneck in the middle; you can hear the seams, you cannot
 ...
 *
 * The API key is local and the client is not dumb: the tools the voice agent
 * calls are the SAME Strands tool objects this process already has mounted, so
 * a spoken "dig straight down" moves the actual bot and answers in the same
 * breath — no tool_call/tool_result bridge over a wire at all.
 *
 * ── the lessons that cost live debugging ────────────────────────────────────
 * Everything below only shows up on a real call:
 *
 ...
 * suppression is the opt-IN (`VOICE_HALF_DUPLEX=1`, `--half-duplex`), for
 * the one setup where physics beats us: built-in speaker into built-in mic, no
 * OS acoustic echo cancellation, where the bot transcribes itself as your next
 ...
 */
export const BARGE_MARGIN = Number(process.env.VOICE_BARGE_MARGIN || 2.5)
...
export const BARGE_FLOOR = Number(process.env.VOICE_BARGE_FLOOR || 0.05)
/** Consecutive loud frames needed. At 20 ms a frame, 3 ≈ 60 ms — a syllable. */
export const BARGE_FRAMES = Number(process.env.VOICE_BARGE_FRAMES || 3)
 ...
 * syllables clear any margin and the bot interrupts itself on every sentence —
 ...
 */
export const ECHO_LEARN_FRAMES = Number(process.env.VOICE_ECHO_LEARN || 10)
 ...
 */
export const TURN_STALL_MS = Number(process.env.VOICE_TURN_STALL_MS || 8000)
...
export function resolveVoice(want?: string): string {
  const v = String(want || process.env.VOICE_NAME || '').toLowerCase()
...
export function resolveNoiseReduction(): 'near_field' | 'far_field' | null {
  const v = String(process.env.VOICE_NOISE_REDUCTION || 'near_field').toLowerCase()
  if (v === 'off' || v === 'none' || v === '0') return null
  return v === 'far_field' ? 'far_field' : 'near_field'
}

/**
 * Is the mic allowed to stream while the bot is talking? Default YES.
 *
 ...
 * VOICE_FULL_DUPLEX=1 remain as explicit yeses (they used to be the only
 * way), VOICE_HALF_DUPLEX=1 / `--half-duplex` is the opt-out for a machine
 * whose speaker feeds its own microphone.
 */
export function resolveFullDuplex(explicit?: boolean): boolean {
  if (typeof explicit === 'boolean') return explicit
  if (process.env.VOICE_FULL_DUPLEX === '1') return true
  return process.env.VOICE_HALF_DUPLEX !== '1'
}
    ...
    session: {
      ...
      audio: {
        input: {
          ...
          // VOICE_NOISE_REDUCTION=far_field for a speakerphone across a
 ...
 * bot that narrates its tool use and speaks in bullet points.
 */
export function buildVoiceInstructions(extra?: string): string {
  const parts = [
    'You are a Minecraft bot with a voice, on a live call with the player, made of a real ' +
    'mineflayer body in their world. Be warm, playful and brief — one or two sentences unless asked ' +
    'for more. Never narrate tool use, never read out coordinates digit by digit; do the thing and ' +
    'say what happened. Your tools ARE your body: walking, digging, building, fighting, seeing ' +
    '(capture_view), finding players (find_player — never ask where someone is). "Come here" means ' +
    'the person speaking. If a job takes a while, say you are starting it, then report when done.',
  ]
...
export interface RealtimeCallOptions {
  ...
  /** Stream the mic even while the bot is talking, so the model's own VAD can
...
export class RealtimeCall {
  ...
  /**
   * #40: an ASK that arrived while the model was already answering.
   *
   * Every rail here used to fire `response.create` blind — the briefing drain
   * every 2s, a tool result, the stall nudge — and the API answers a second one
   * with "conversation already has an active response". The briefing was then
   * silently unanswered: the item was in the conversation, nothing spoke it.
   * One deferred ask is enough (they collapse into one answer at the next turn
   * boundary), so this is a flag, not a queue.
   */
  private wantResponse = false
  /** How many asks were deferred instead of fired blind (observability). */
  public deferredResponses = 0
  ...
  /** The door is open: someone talked over the bot, so stop gating this turn. */
  ...
  /**
   * Ask for an answer — the ONLY place a response.create leaves this class.
   * While a response is active the ask is remembered and fired on response.done,
   * so a briefing that lands mid-sentence is answered late instead of erroring.
   * Returns true if the ask went out now, false if it was deferred.
   */
  private requestResponse(): boolean {
    if (this.responseActive) { this.wantResponse = true; this.deferredResponses++; return false }
    this.send({ type: 'response.create' })
    return true
  }

  /** True while the model is answering — a caller may want to say so out loud. */
  get busy(): boolean { return this.responseActive }
  ...
  async start(): Promise<void> {
    ...
    const model = this.opts.model || process.env.VOICE_MODEL || DEFAULT_REALTIME_MODEL
  ...
  /**
   * A mic frame: gate it if the bot is talking, else append it upstream.
   *
  ...
  private onUpstream(data: any) {
    ...
    switch (msg.type) {
      ...
      case 'response.done': {
        ...
        this.finishTurn()
        // #40: whatever arrived mid-answer gets its turn now. The items are
        // already in the conversation, so one create covers all of them.
        if (this.wantResponse) { this.wantResponse = false; this.send({ type: 'response.create' }) }
        break
      }
      case 'error': {
        ...
        // #40 belt and braces: if the server says a response is already active,
        // our own flag was wrong — remember the ask instead of losing it.
  ...
  /**
   * Type into a live call. The bot hears the text and answers out loud — the
