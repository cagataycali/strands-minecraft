 ...
 * no AEC on a laptop pipe. With semantic VAD, a voice agent talking through the
 ...
 * and it is an OPT-IN (`VOICE_HALF_DUPLEX=1`, `--half-duplex`), not the
 ...
 * setup almost everyone uses, talking over the bot did nothing at all.
 *
...
/** Long-lived streaming child (recorder/player) — lifetime is the caller's. */
function stream(bin: string, args: string[], opts?: { env?: NodeJS.ProcessEnv }): ChildProcess {
  return spawn(bin, args, { env: opts?.env, stdio: ['pipe', 'pipe', 'pipe'] })
}
 ...
 * recorder can hear the bot and never answer it, which is worse than a clear
 * "install this first".
 */
export function detectBackend(): AudioBackend | null {
  if (process.env.VOICE_BACKEND === 'sox' || process.env.VOICE_BACKEND === 'ffmpeg') {
    return process.env.VOICE_BACKEND as AudioBackend
  }
 ...
 *
 * VOICE_INPUT picks a non-default device (ffmpeg avfoundation index like
 ...
 * reads as the bot thinking for a second after it has already answered.
 */
...
export function openMic(
  ...
  const { bin, args } = recordArgs(backend, process.env.VOICE_INPUT)
 ...
 * the bot still audible" is derived from how much audio has been handed over
